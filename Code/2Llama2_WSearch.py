...
history_file_path = "../Laama2_RunHistory_LoRA_WSearch.csv"
...
train_indices = random.sample(all_indices, 10000)
...
#         print(f"File '{file_path}' already exists.")
    ...
    with open(file_path, 'a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        this_row=["code_docstring_corpus_data_10ktrainrows", "10000", "Central", "NA", model_id, TRN_PARAM, LORA_TARGET_MODULES, lora_r ,before_time, after_time, num_train_epochs, len(test_dataset), metric_results['BLEU'],  metric_results['METEOR'],metric_results['ROUGE']]
...
#RQ1: Identify "Weight Matrices" 
list_lora_r = [32,32,32]#16, 64, 64, 64, 64,32,32,32, #33M param
list_LORA_TARGET_MODULES = [["q_proj","o_proj"],["k_proj","o_proj"],["v_proj","o_proj"]]#["q_proj","v_proj","k_proj", "o_proj",],["q_proj"],["v_proj"],["k_proj"], ["o_proj"],["q_proj","k_proj"],["q_proj","v_proj"],["k_proj","v_proj"],
for i_exp in range(len(list_lora_r)):
    LORA_TARGET_MODULES = list_LORA_TARGET_MODULES[i_exp]
    lora_r = list_lora_r[i_exp]
    print("\n\n----------------------------------------------------------------------")
    print("Beginning experiment for W=", LORA_TARGET_MODULES, "with r=", lora_r)
    ...
    print("SEeeeeee!! experiment completed for W=", LORA_TARGET_MODULES, "with r=", lora_r)
