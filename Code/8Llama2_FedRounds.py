...
def get_model_tobe_trained(prevmodel_dir):
    ...
    # print(compute_dtype) #Half precision floating-point format Sign bit: 1 bit, Exponent: 5 bits, Mantissa: 10 bits.
...
#     model = merge_lora2base(prevmodel_dir)
#     ##Check if below line will work?????????????????
# #     model = AutoModelForCausalLM.from_pretrained(model, quantization_config=bnb_config, use_cache=False, device_map=device_map)
    merged_model_dir = prevmodel_dir+"../merged_model"
#     model.save_pretrained(merged_model_dir,safe_serialization=True)
#     del model
    model = AutoModelForCausalLM.from_pretrained(merged_model_dir, quantization_config=bnb_config, use_cache=False, device_map=device_map)
#     delete_directory(merged_model_dir)
...
for this_round in range(1,num_rounds+1):
    ...
    #Update the base model name in adapter_config.json file 
#     update_basemodel_inAdaCfg(adapter_dir)
